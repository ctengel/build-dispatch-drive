...
def draw_hud(surf, game):
    cam = game.camera
    pygame.draw.rect(surf, C.COL_HUD_BG, (0, 0, cam.w, 30))
    pygame.draw.rect(surf, C.COL_HUD_BG, (0, cam.h - 26, cam.w, 26))
    ...
    if t is not None:
        ...
        if t.note:
            info += "  [%s]" % t.note
        left += info
    text(surf, left, (10, 7), C.COL_HUD_TXT, 20)
    text(surf, game.mode_hints(), (10, cam.h - 20), C.COL_HUD_DIM, 18)

    # transient messages, newest at the top
    y = 40
    for msg, expiry in game.messages:
        text(surf, msg, (cam.w // 2, y), C.COL_MSG, 22, center=True)
        y += 24
    # SPAD banner
    for t in game.world.trains:
        if t.spad and (game.time * 2) % 1 < 0.6:
            text(surf, "SIGNAL PASSED AT DANGER - train #%d" % t.id,
                 (cam.w // 2, cam.h // 2 - 120), C.COL_SPAD, 30, center=True)
